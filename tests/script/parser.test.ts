...
import { type ASTChunk, ASTType, Lexer, Parser } from '../../src/script/index'
	...
	describe('default scripts', () => {
		fs.readdirSync(testFolder).forEach((file) => {
			...
			test(path.basename(filepath), () => {
				...
				const payload = parser.parseChunk() as ASTChunk

				expect(payload.toString()).toMatchSnapshot()
				expect(
					payload.namespaces.reduce((result: Record<string, number>, item: any) => {
