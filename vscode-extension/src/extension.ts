...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register commands
    const runScriptCommand = vscode.commands.registerCommand('npcs.runScript', async (uri?: vscode.Uri) => {
        await runNPCScript(uri);
    });

    const debugScriptCommand = vscode.commands.registerCommand('npcs.debugScript', async (uri?: vscode.Uri) => {
    ...
    const validateActive = (doc: vscode.TextDocument) => {
        if (doc.languageId !== 'npcs') return;
        try {
...
async function runNPCScript(uri?: vscode.Uri) {
...
async function debugNPCScript(uri?: vscode.Uri) {
    const activeEditor = vscode.window.activeTextEditor;
    const targetUri = uri || activeEditor?.document.uri;
...
function provideHoverInfo(word: string): vscode.Hover | undefined {
...
function provideCompletionItems(document: vscode.TextDocument, position: vscode.Position): vscode.CompletionItem[] {
    ...
    builtinFunctions.forEach(func => {
        ...
        if (hoverInfo && hoverInfo.contents[0]) {
            item.documentation = hoverInfo.contents[0] as vscode.MarkdownString;
        }
        items.push(item);
    });
...
function validateDocument(document: vscode.TextDocument, diagnosticCollection: vscode.DiagnosticCollection) {
    ...
    type BlockType = 'if' | 'for' | 'do' | 'function';
    const stack: Array<{ type: BlockType; line: number } > = [];
    ...
    const stripLineComments = (line: string): string => {
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            else if (!inDouble && ch === '\'' && prev !== '\\') inSingle = !inSingle;
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        const pushBlock = (type: BlockType) => stack.push({ type, line: lineIndex });
        const popBlock = (expected: BlockType, endLen: number) => {
            const top = stack.pop();
        ...
        if (forOpen.test(line)) { pushBlock('for'); continue; }
        if (doOpen.test(line)) { pushBlock('do'); continue; }
        if (funcOpen.test(line)) { pushBlock('function'); continue; }
        ...
        if (endIf.test(line)) { popBlock('if', 6); continue; }
        if (endFor.test(line)) { popBlock('for', 7); continue; }
        if (endLoop.test(line)) { popBlock('do', 4); continue; }
        if (endFunc.test(line)) { popBlock('function', 12); continue; }
...
class NpcsFoldingProvider implements vscode.FoldingRangeProvider {
    provideFoldingRanges(document: vscode.TextDocument, context: vscode.FoldingContext, token: vscode.CancellationToken): vscode.ProviderResult<vscode.FoldingRange[]> {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (inBlockComment) {
                const endIdx = raw.indexOf('*/');
                if (endIdx >= 0) { raw = raw.slice(endIdx + 2); inBlockComment = false; } else { continue; }
            }
            const startIdx = raw.indexOf('/*');
            if (startIdx >= 0) {
                const endIdx = raw.indexOf('*/', startIdx + 2);
                if (endIdx >= 0) raw = raw.slice(0, startIdx) + raw.slice(endIdx + 2); else { raw = raw.slice(0, startIdx); inBlockComment = true; }
            }
            ...
            if (/^do\b/i.test(line)) { stack.push({ type: 'do', line: i }); continue; }
            ...
            if (/^end\s+if\b/i.test(line)) {
                const top = stack.pop();
                if (top && top.type === 'if' && i > top.line) ranges.push(new vscode.FoldingRange(top.line, i));
                continue;
            }
            if (/^end\s+for\b/i.test(line)) {
                const top = stack.pop();
                if (top && top.type === 'for' && i > top.line) ranges.push(new vscode.FoldingRange(top.line, i));
                continue;
            }
            if (/^loop\b/i.test(line)) {
                const top = stack.pop();
                if (top && top.type === 'do' && i > top.line) ranges.push(new vscode.FoldingRange(top.line, i));
                continue;
            }
            if (/^end\s+function\b/i.test(line)) {
                const top = stack.pop();
                if (top && top.type === 'function' && i > top.line) ranges.push(new vscode.FoldingRange(top.line, i));
                continue;
            }
        }
