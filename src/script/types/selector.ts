...
export enum SelectorType {
	...
	ImportCodeSeparator = 'ImportCodeSeparator',
	ElseIf = 'ElseIf',
	Then = 'Then',
	Else = 'Else',
	In = 'In',
...
export function createSelectorGroup(name: string, selectors: Selector[]): SelectorGroup {
	const selectorsWithValue = selectors.filter((item) => item.data.value !== undefined)
	const casesWithValue = selectorsWithValue
		.map((selector) => {
			return `case '${selector.data.value}': return token.type === '${selector.data.type}';`
		})
		.join('\n')
	const selectorsWithoutValue = selectors.filter((item) => item.data.value === undefined)
